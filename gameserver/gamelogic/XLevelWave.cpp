	...
	case TYPE_MONSTERINFO:
		{
			...
			m_Monsters.insert(make_pair<int, Vector3>(index, Vector3(x, y, z)));
			m_MonsterRotation.insert(make_pair<int, float>(index, (float)atof(strFloats[4].c_str())));
		}
		break;
	case TYPE_SCRIPT:
		{
...
void XLevelSpawner::Init()
{
	...
	for (auto iter = m_waveInfos.begin(); iter != m_waveInfos.end(); ++iter)
	{
		for(auto j = iter->second.m_staticLevelWaveInfo.m_PreWaves.begin(); j != iter->second.m_staticLevelWaveInfo.m_PreWaves.end(); ++j)
		{
			...
			{
				iter->second.m_preLevelWaveInfos.insert(std::make_pair<int, XLevelWaveInfo*>(*j, &(find->second)));
			}
			else
			{
