...
public:
	...
	virtual bool OnLine(char **Fields, int FieldCount)
	{
		...
		if (!Parse(Fields[m_ColMap[1]], row.GuildExpNeed)) return false; ++m_ColNum;
		if (!Parse(Fields[m_ColMap[2]], row.GuildNumber)) return false; ++m_ColNum;
		if (!Parse(Fields[m_ColMap[3]], row.PokerTimes)) return false; ++m_ColNum;
		if (!Parse(Fields[m_ColMap[4]], row.PokerReplaceTimes)) return false; ++m_ColNum;
		if (!Parse(Fields[m_ColMap[5]], row.GuildSign)) return false; ++m_ColNum;
		if (!Parse(Fields[m_ColMap[6]], row.GuildWelfare)) return false; ++m_ColNum;
		if (!Parse(Fields[m_ColMap[7]], row.GuildStore)) return false; ++m_ColNum;
