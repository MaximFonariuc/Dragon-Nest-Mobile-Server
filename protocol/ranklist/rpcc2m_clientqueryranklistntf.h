#ifndef __RPCC2M_CLIENTQUERYRANKLISTNTF_H__
#define __RPCC2M_CLIENTQUERYRANKLISTNTF_H__

// generate by ProtoGen at date: 2016/11/1 22:39:07

#include "crpc.h"
#include "pb/project.pb.h"

using namespace KKSG;

#define RPCC2M_CLIENTQUERYRANKLISTNTF_TYPE 39913

class RpcC2M_ClientQueryRankListNtf : public CRpc
{
private:

	RpcC2M_ClientQueryRankListNtf():CRpc(RPCC2M_CLIENTQUERYRANKLISTNTF_TYPE)
	{
        m_dwTimeout = 5000;
		m_ArgPtr = &m_oArg;
		m_ResPtr = &m_oRes;
	}

public:
	static RpcC2M_ClientQueryRankListNtf *CreateRpc()
	{
		return new RpcC2M_ClientQueryRankListNtf;
	}

	virtual RpcEndType GetEndType() const;
	virtual void Process(UINT32 dwConnID);

private:
	// implemention as client
	void OnReply(const ClientQueryRankListArg &roArg, const ClientQueryRankListRes &roRes, const CUserData &roUserData);
	void OnTimeout(const ClientQueryRankListArg &roArg, const CUserData &roUserData);

	// implemention as server
	void OnCall(const ClientQueryRankListArg &roArg, ClientQueryRankListRes &roRes);
	void OnDelayReplyRpc(const ClientQueryRankListArg &roArg, ClientQueryRankListRes &roRes, const CUserData &roUserData);

	virtual void OnReplyTimeout() 
	{ 
		OnTimeout(m_oArg, m_oUser); 
	}

	virtual void OnDelayRpc(const CUserData &userData)
	{
		OnDelayReplyRpc( m_oArg, m_oRes, userData);
	}

	virtual CRpc *NewRpc() 
	{
		return CreateRpc();
	}

public:
	ClientQueryRankListArg m_oArg;
	ClientQueryRankListRes m_oRes;
};

#endif
