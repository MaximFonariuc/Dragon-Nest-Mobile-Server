...
bool CPlatformFriendMgr::LoadFromDB()
{
	...
	while(poRes->FetchRow())
	{
		...
		m_platFriendInfo.insert(std::make_pair<std::string, PlatFriendList>(openid, list));
	}
	poRes->Release();
...
{
	m_reqCSList.insert(std::make_pair<std::string, UINT32>(const_cast<std::string&>(openid), time(0)));
}
...
bool PlatFriendList::LoadFromDb(const std::string& info)
{
	...
	for (int i = 0; i < tempInfo.platfriendsinfo_size(); ++i)
	{
		...
		m_platFriendList.insert(std::make_pair<std::string, PlatUserInfo>(friendInfo.openid, friendInfo));
	}
...
bool PlatFriendList::SyncPlatFriendListFromClient(const KKSG::SyncPlatFriend2MSData& data)
{
	...
	for (int i = 0; i < data.friendinfo_size() && i < 200; ++i)
	{
		...
		if (find == m_platFriendList.end())
		{
			...
			m_platFriendList.insert(std::make_pair<std::string, PlatUserInfo>(temp.openid, temp));
			change = true;
		}
		else
		{
